...
defaultoptions = struct('FrangiScaleRange', [1 10], 'FrangiScaleRatio', 2, 'FrangiAlpha', 0.5, 'FrangiBeta', 0.5,'FrangiHu',30, 'FrangiC', 500, 'verbose',true,'BlackWhite',true);
...
for i = 1:length(sigmas),
    ...
    %Addition: Compute additional F function
    Pmax = max(max(max(I)));
    Frangi_F = 0.5*(1+(2/pi)*atan((I - options.FrangiHu)*2/Pmax));
    
    %Compute Vesselness function
    Voxel_data = expRa.* expRb.* expS.*Frangi_F;
    
    % Free memory
    clear expRa expRb expRc Frangi_F;
